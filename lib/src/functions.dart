...
Future<List<double>> listCompare({
  ...
  await Future.wait(list.map((otherSrc) async {
    results[list.indexOf(otherSrc)] = await compareImages(src1: target, src2: otherSrc, algorithm: algorithm);
  }));

  return results;
}

Image _getImageFromDynamic(var src) {
  ...
  if (src is File) {
    ...
    throw UnsupportedError("The source, ${src.runtimeType}, passed in is unsupported");
  }
